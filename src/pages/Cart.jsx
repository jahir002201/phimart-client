...
const Cart = () => {
    ...
    const fetchOrCreateCart = async () => {
      if (cartId) {
        try {
          ...
          console.warn("Cart not found or invalid, creating a new one.");
  ...
  const handleUpdateQuantity = async (itemId, newQuantity) => {
    ...
    setCart({
      ...cart,
      items: updatedItems,
      total_price: updatedItems.reduce((sum, i) => sum + i.total_price, 0),
    });

    try {
      await authApiClient.patch(`/carts/${cartId}/items/${itemId}/`, { quantity: newQuantity });
    } catch (err) {
